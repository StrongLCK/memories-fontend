...
const PostDetails = () => {
    ...
    // get the post by id=> return post
    useEffect(() => {
        dispatch(getPost(id));
    }, [id]);

    // find recommendedPosts by tags, post was the post above, return posts
    useEffect(() => {
        if (post) {
    ...
    //console.log(posts);
    const recommendedPosts = posts.filter((a) => a._id !== post._id);
                    ...
                    <div className={classes.recommendedPosts}>
                        {recommendedPosts.map(({ title, name, message, likes, selectedFile, _id }) => (
                            <div style={{ margin: '20px', cursor: 'pointer' }} onClick={() => openPost(_id)} key={_id}>
                                ...
                                <img src={selectedFile} alt="noSelectedFile" width="200px" />
                            </div>
                    ...
                    <div className={classes.recommendedPosts}>
                        {recommendedPosts.map((post) => (
                            <div style={{ margin: '20px', cursor: 'pointer' }} onClick={() => openPost(post._id)} key={post._id}>
                                ...
                                <img src={post.selectedFile} alt="noSelectedFile" width="200px" />
                            </div>
                        ))}
                    </div>
                </div>
